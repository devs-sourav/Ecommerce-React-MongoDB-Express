...
const ViewSubCategory = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:8000/api/v1/product/allsubcategory');
  ...
  const confirmDelete = async (_id) => {
    try {
      const response = await axios.post('http://localhost:8000/api/v1/product/deletesubcategory', { id: _id });
  ...
  const handleIdle = async (_id) => {
    try {
      const response = await axios.post('http://localhost:8000/api/v1/product/idlesubcategory', { id: _id });
      if (response.data.success) {
  ...
  const approveActive = async (_id) => {
    try {
      const response = await axios.post('http://localhost:8000/api/v1/product/approvesubcategory', { id: _id, isActive: true });
